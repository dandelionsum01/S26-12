...
void Booking::updateCapacity(const string& pkgID)
{
    ...
    while (getline(fileIn, line))
    {
        ...
        if (trimWS(id) == pkgID)
        {
            ...
            fileOut << id << "," << from << "," << to << "," << price << "," << hotel << "," << remaining << "," << depDate << "," << expiry << "," << category << "\n";
            updated = true;
        }
        else
        {
...
void Booking::makeBooking(const string& customerUsername)
{
    ...
    cout << "\n========== Available Packages ==========\n";
    cout << left << setw(14) << "ID" << setw(12) << "From" << setw(12) << "To" << setw(10) << "Price" << setw(17) << "Hotel" << setw(10) << "Seats Left" << setw(12) << "Departure" << setw(10) << "Category" << "\n";
    cout << string(97, '-') << "\n";
    bool anyAvailable = false;
    for (int i = 0; i < pkgCount; ++i)
    {
        if (packages[i].expiry < today)
        {
            continue;
        }
        ...
        anyAvailable = true;
        cout << left << setw(14) << packages[i].id << setw(12) << packages[i].from << setw(12) << packages[i].to << setw(10) << ("Rs." + to_string(packages[i].price)) << setw(17) << packages[i].hotel << setw(10) << seatsLeft << setw(12) << packages[i].depDate << setw(10) << packages[i].category << "\n";
    }
    if (!anyAvailable)
    {
    ...
    if (bf.is_open())
    {
        ...
        while (getline(bf, line))
        {
            ...
            getline(ss, uname, ',');
            getline(ss, pkgID, ',');
            if (trimWS(uname) == customerUsername && trimWS(pkgID) == chosen.id)
            {
    ...
    while (!validCard && attempts < 3)
    {
        ...
        {
            attempts++;
            cout << "Invalid card number! " << (3 - attempts) << " attempt(s) remaining.\n";
        }
    }
    if (!validCard)
    {

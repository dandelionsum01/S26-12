...
int CustomPackage::calculateBill()
{
    ...
    if (cities != nullptr)
    {
        ...
        string startLower = toLowerCity(getStart());
        string destLower = toLowerCity(getDestination());

        for (int i = 0; i < count; i++)
        {
            if (toLowerCity(cities[i].name) == startLower)
            {
                fromCity = cities[i];
                foundFrom = true;
            }
            if (toLowerCity(cities[i].name) == destLower)
            {
                toCity = cities[i];
                foundTo = true;
            }
...
void CustomPackage::enterDetails(const string &customerUsername)
{
    ...
    {
        cout << "City name(s) cannot be empty. Aborting.\n";
        return;
    }
    ...
    cin >> date;
    {
        bool ok = (date.length() == 10 && date[4] == '-' && date[7] == '-');
        if (ok)
        {
        ...
        if (!ok)
        {
            cout << "Invalid date format. Use YYYY-MM-DD. Aborting.\n";
            return;
        }
    }
